...
class plgHikashoppaymentIdpay extends hikashopPaymentPlugin
{
    ...
    /**
     * plgHikashoppaymentIdpay constructor.
     * @param $subject
     * @param $config
    ...
    public function onAfterOrderConfirm(&$order, &$methods, $method_id)
    {
        ...
        $this->order_log($order->order_id, "idpay_id:$result->id");
    ...
    public function onPaymentNotification(&$statuses)
    {
        ...
        if (!empty($pId) && !empty($pOrderId) && $pOrderId == $order_id) {
            if ($pStatus == 10) {
                ...
                $db = JFactory::getDBO();
                $isNotDoubleSpending = $this->isNotDoubleSpending($db, $pOrderId, $pId);

                if ($isNotDoubleSpending == false) {
                    $msg = $this->idpay_get_failed_message($pTrackId, $order_id, 0);
    ...
    public function isNotDoubleSpending($reference, $order_id, $transaction_id)
    {
        $idpay_id = "idpay_id:$transaction_id";
        $db = $reference;
        $sql = 'SElECT history_reason FROM ' . "#__hikashop_history" . '  WHERE history_order_id= ' . $order_id .
            ' AND history_reason = "' . $idpay_id . '"';
        $db->setQuery($sql);
        $db->execute();
